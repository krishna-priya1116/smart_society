...
class VehicleTracking(models.Model):
    ...
    default=lambda self :self.env['security.guard'].search([('security_id','=',self.env.user.id)],limit=1))
    # not assinging
    ...
    def assign_parking_type(self):
        for record in self:
            if record.person_type=='resident' or record.resident_id:
                record.parking_type='resident_parking'

            if record.person_type=='visitor':
